$(document).ready(function () {
    ...
    $('#showRegister').click(function () {
        ...
        $('#loginText').text('Kattints ide a regisztrációhoz!');
    });
    ...
    $('#showLogin').click(function () {
        ...
        $('#loginText').text('Kattints ide a belépéshez!');
    });

    $('#loginText').click(function (e) {
        e.preventDefault();

        if ($('#loginText').text().trim() === 'Kattints ide a regisztrációhoz!') {
...
$(document).ready(function () {
    ...
    $.get('/User/CheckSession')
        .done(function (response) {
            if (response.userID !== -1) {
                ...
                $('#contactBackButton').attr('href', 'dashboard.html');
                $('#productBackButton').attr('href', 'dashboard.html');
                $('#orderBackButton').attr('href', 'dashboard.html');
                $('#invoiceBackButton').attr('href', 'dashboard.html');
                $('#financeBackButton').attr('href', 'dashboard.html');
                $('#reportBackButton').attr('href', 'dashboard.html');
            } else {
                $('#contactBackButton').attr('href', 'index.html');
                $('#productBackButton').attr('href', 'index.html');
                $('#orderBackButton').attr('href', 'index.html');
                $('#invoiceBackButton').attr('href', 'index.html');
                $('#financeBackButton').attr('href', 'index.html');
                $('#reportBackButton').attr('href', 'index.html');
            ...
            <div class="col">
                <div class="card h-100 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text fw-bold">${product.productPrice} €</p>
                        ${userRole === 'admin' ? `
    ...
    form.submit(function (e) {
        ...
        if (!allowedProducts.hasOwnProperty(productName)) {
            alert('A kiválasztott termékhez nem található kép URL. Kérlek, válassz egy létező terméket!');
            return;
        }
    ...
    function renderOrders(orders) {
        ...
        orders.forEach(order => {
            ...
            if (!(order.status === 'Rendelés zárolva' && (role === 'admin' || role === 'customer'))) {
                actionButtons = `
                <button class="btn btn-warning btn-sm edit-order" data-id="${order.orderID}" style="width: 127%; margin-bottom: 5px;">
                    ✏️ Szerkesztés
                </button>
                <button class="btn btn-danger btn-sm delete-order" data-id="${order.orderID}" style="width: 127%;">
                    🗑 Törlés
                </button>
            `;
            }
    ...
    $('#exportPDF').click(function () {
        ...
        const ordersLocked = localStorage.getItem("ordersLocked") === "true";
        const confirmMessage = ordersLocked
            ? "Szeretnéd elmenteni a rendeléseket PDF-be?"
            : "Szeretnéd lezárni a rendeléseket és menteni PDF-be?";
    ...
    function renderFinance(entries) {
        ...
        $('#income-amount').text(totalIncome + ' €');
        $('#expense-amount').text(totalExpense + ' €');
        $('#profit-amount').text((totalIncome - totalExpense) + ' €');
    }
