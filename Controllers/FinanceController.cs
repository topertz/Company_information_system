...
public class FinanceController : Controller
{
    ...
    [HttpPost]
    public IActionResult CreateFinance([FromForm] string month, [FromForm] int income, [FromForm] int expense)
    {
        using var connection = DatabaseConnector.CreateNewConnection();
        string sql = "INSERT INTO Finance (Month, Income, Expense) VALUES (@Month, @Income, @Expense)";
        using var cmd = new SQLiteCommand(sql, connection);
    ...
    [HttpPost]
    public IActionResult UpdateFinance([FromForm] int financeID, [FromForm] string? month, [FromForm] int? income, [FromForm] int? expense)
    {
        using var connection = DatabaseConnector.CreateNewConnection();
        string sql = @"
        UPDATE Finance 
        SET 
            Month = COALESCE(NULLIF(@Month, ''), Month),
    ...
    ORDER BY Month";
        ...
        var monthlyData = new List<(string Month, int Income)>();
        ...
        foreach (var item in monthlyData)
        {
            ...
            // Update
            string updateSql = @"
        UPDATE Finance SET Income = @Income, Expense = @Expense WHERE Month = @Month";
            ...
            updateCmd.Parameters.AddWithValue("@Month", item.Month);
            ...
            if (rowsAffected == 0)
            {
                // Insert
                string insertSql = @"
            INSERT INTO Finance (Month, Income, Expense) VALUES (@Month, @Income, @Expense)";
