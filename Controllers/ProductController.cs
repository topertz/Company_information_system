...
public class ProductController : Controller
{
    ...
    {
        using var connection = DatabaseConnector.CreateNewConnection();
        string sql = "INSERT INTO Products (ProductName, ProductPrice, ImageUrl) VALUES (@ProductName, @ProductPrice, @ImageUrl)";
        using var cmd = new SQLiteCommand(sql, connection);
    ...
    [HttpPost]
    public IActionResult UpdateProduct([FromForm] int productID, [FromForm] string? productName, [FromForm] int? productPrice, [FromForm] string imageUrl)
    {
        using var connection = DatabaseConnector.CreateNewConnection();
        string sql = @"
        UPDATE Products 
        ...
        cmd.Parameters.AddWithValue("@ImageUrl", (object?)imageUrl ?? "");
    ...
    public IActionResult SeedProducts()
    {
        using var connection = DatabaseConnector.CreateNewConnection();
        string sql = @"
    INSERT OR IGNORE INTO Products (ProductID, ProductName, ProductPrice, ImageUrl) VALUES
