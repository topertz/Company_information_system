...
public class OrderController : Controller
{
    ...
    [HttpPost]
    public IActionResult CreateOrder([FromForm] string customerName, [FromForm] string customerEmail, [FromForm] string customerAddress, [FromForm] int houseNumber, [FromForm] int productID, [FromForm] int price, [FromForm] int amount, [FromForm] string status, [FromForm] string orderDate)
    {
        using (var connection = DatabaseConnector.CreateNewConnection()) 
        {
            string sql = @"INSERT INTO ""Order"" (CustomerName, CustomerEmail, CustomerAddress, HouseNumber, ProductID, Price, Amount, Status, OrderDate)
                   VALUES (@CustomerName, @CustomerEmail, @CustomerAddress, @HouseNumber, @ProductID, @Price, @Amount, @Status, @OrderDate)";
            using (var cmd = new SQLiteCommand(sql, connection))
            {
